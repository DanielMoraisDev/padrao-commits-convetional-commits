...
## 🛠️ Como instalar o arquivo `prepare-commit-msg.sh` para validar mensagens de commits com conventional commits

### Passo 1: Localize o arquivo `prepare-commit-msg.sh` 📂

O arquivo `prepare-commit-msg.sh` deve estar disponível no repositório do seu projeto ou em um diretório específico. Certifique-se de que ele está acessível. Se não estiver, faça o download ou clone o repositório onde ele está localizado.

### Passo 2: Copie o arquivo `prepare-commit-msg.sh` para o diretório `.git/hooks` do seu projeto 📋

Copie o arquivo `prepare-commit-msg.sh` para o diretório `.git/hooks` e renomeie-o para `prepare-commit-msg` (sem extensão):

```bash
cp caminho/para/prepare-commit-msg.sh .git/hooks/prepare-commit-msg
```
